...
import { createFcmSender } from './lib/fcm';
...
const ctx = { db, config, media: new MediaStorage(db, config.dataDir), usage: new UsageLedger(db), push, fcm };
...
createApp(ctx).listen(config.port, '0.0.0.0', () => {
  ...
  console.log(
    `providers → openrouter: ${config.openRouterKey ? 'on' : 'off'}, fal: ${config.falKey ? 'on' : 'off'}, revenuecat: ${config.revenueCatSecret ? 'on' : 'off'}, apns: ${push ? 'on' : 'off'}, fcm: ${fcm ? 'on' : 'off'}${config.devPlanOverride ? `, dev plan: ${config.devPlanOverride}` : ''}`
  );
});
